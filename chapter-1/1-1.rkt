...
;;; Exercise 1.7
; The `good-enough?` test fails for both very large and very small numbers, albeit, for different
; reasons.

; With large numbers, floating point arithmetic causes the difference between numbers to sometimes
; be larger than the delta we defined in our test. This means that due to a rounding error, we will
; never pass the test. and will always return the same number (that is larger than .001)

; With small numbers, the test fails because .001 as a delta isn't adequate for finding the square
; root of numbers smaller than .001. It will return a "correct" result way too early.

; Although the nature of the problems is different, they can both be solved with the same solution.
; If we make `good-enough?` adaptive, we can deal with the difference between our current guess and
; our previous guess. This will allow us to drill in with much more precision, as well as to not rely
; on `x` anymore, which was causing our rounding error problems with larger numbers.
